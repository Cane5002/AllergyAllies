  ...
  useEffect(() => {
    ...
    const findPatient = async () => {
      if (email){
        ...
        const patientObj = await axios.get(`${process.env.APP_MANIFEST.database.IP}:${process.env.APP_MANIFEST.database.Port}/api/findPatient/${email}`)
    ...
    const findPractice = async () => {
      //replace with your IP address, find quickly from "Metro waiting on exp://<ip>:port" under QR code
      const practiceObj = await axios.get(`${process.env.APP_MANIFEST.database.IP}:${process.env.APP_MANIFEST.database.Port}/api/practice/${patient.practiceID}`)
    ...
    const findProtocol = async () => {
      //replace with your IP address, find quickly from "Metro waiting on exp://<ip>:port" under QR code
      const protocolObj = await axios.get(`${process.env.APP_MANIFEST.database.IP}:${process.env.APP_MANIFEST.database.Port}/api/getProtocol/${patient.practiceID}`)
